global_dict = {


   # copy files and data starting at this directory
    'src_root':  '/home/jtf/gith/www_source/projects-source/projects/',
    # copy files and data to tree at this root, stuff will start one dir below
    'dest_root': '/home/jtf/www/projects',
    # root for html paths where to find css (in "/css subdir")
    'public': '/projects/public/',
    # html root
    'html_root': '/projects/',
    # root for this tree ")
    'html_top': '/projects/',
    # content file extension
    'content_ext': '.md',
    # generate warning if thumbnail not found
    'warn_thumbnail':False,
    # find templates in this dir
    'template_dir': '/home/jtf/gith/www_source/projects-source/templates',
    'date': '0/0/1970',
    'title': 'default_title',
    'name': 'Jonathan Foote',
    'hostname': 'rotormind.com',
    'keywords': 'unset keywords'
}

